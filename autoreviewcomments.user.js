...
with_jquery(function ($) {
	StackExchange.ready(function () {
		...
		function CheckForNewVersion(popup) {
				...
				if (LastUpdateCheckDay == null) { //first time visitor
					ShowMessage(popup, 'Please read this!', 'Thanks for installing this script. \
                            ...
                            For other options, please see the README at <a href="https://github.com/Benjol/SE-AutoReviewComments" target="_blank">here</a>.', function () {});
				}
					...
					updateCheck(function (newver, oldver, install_url) {
						if (newver != GetStorage('LastVersionAcknowledged')) {
							ShowMessage(popup, 'New Version!', 'A new version (' + newver + ') of the <a href="http://stackapps.com/q/2116">AutoReviewComments</a> userscript is now available, see the <a href="https://github.com/Benjol/SE-AutoReviewComments/releases">release notes</a> for details or <a href="' + install_url + '">click here</a> to install now.', function () {
		...
		var defaultcomments = [{
			...
			Description: 'Please don\'t add "thanks" as answers. Invest some time in the site and you will gain sufficient <a href="http://$SITEURL$/privileges">privileges</a> to upvote answers you like, which is the $SITENAME$ way of saying thank you.'
		}, {
			Name: 'Nothing but a URL (and isn\'t spam)',
			Description: 'Whilst this may theoretically answer the question, <a href="http://meta.stackoverflow.com/q/8259">it would be preferable</a> to include the essential parts of the answer here, and provide the link for reference.'
		}, {
			Name: 'Requests to OP for further information',
			Description: 'This is really a comment, not an answer. With a bit more rep, <a href="http://$SITEURL$/privileges/comment">you will be able to post comments</a>. For the moment I\'ve added the comment for you, and I\'m flagging this post for deletion.'
		}, {
		...
		function Tag(html) {
				...
				var regname = new RegExp(sitename, 'g'),
					regurl = new RegExp('http://' + siteurl, 'g'),
		...
		function ToEditable(el) {
				...
				var txt = $('<textarea />').css('height', 2 * el.height()).css('width', el.css('width')).val(htmlToMarkDown(html));
				BorkFor(el); //this is a hack
				//save/cancel links to add to textarea
				var commands = $('<a>save</a>').click(function () {
					SaveEditable($(this).parent());
					UnborkFor(el);
				}).add('<span class="lsep"> | </span>').add($('<a>cancel</a>').click(function () {
					CancelEditable($(this).parent(), backup);
					UnborkFor(el);
				}));
		...
		function WriteComments(popup) {
			...
			for (var i = 0; i < GetStorage('commentcount'); i++) {
				...
				if (commenttype == 'any' || (commenttype == popup.posttype)) {
					...
					var opt = optionTemplate.replace(/\$ID\$/g, i).replace('$NAME$', GetStorage('name-' + i).replace(/\$/g, '$$$')).replace('$DESCRIPTION$', (showGreeting ? greeting : '') + desc);
					ul.append(opt);
				}
			}
		...
		function AddOptionEventHandlers(popup) {
				...
				popup.find('input:radio').click(function () {
					...
					$(this).parent().addClass('action-selected').find('.action-desc').show();
				});
		...
		function ShowMessage(popup, title, body, callback) {
				...
				var message = $(messageTemplate.replace('$TITLE$', title).replace('$BODY$', html));
		...
		$('#content').delegate('comments-link', 'click', function () {
			...
			var newspan = $('<span class="lsep"> | </span>').add($('<a class="comment-auto-link">auto</a>').click(function () {
				...
				if (!window.VersionChecked && GetStorage('AutoRemote') == 'true') {
					...
					throbber.show();
					LoadFromRemote(GetStorage('RemoteUrl'), function () {
						WriteComments(popup);
						throbber.hide();
					}, function (d, msg) {
						remoteerror.text(msg);
